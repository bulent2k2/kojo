            ...
            case Some(Left(completions)) =>
              val response: pcompiler.Response[List[CompletionInfo]] = pcompiler.askForResponse { () =>
                ...
                completions.foreach { completion =>
                  try {
                    completion match {
                      case pcompiler.TypeMember(sym, tpe, true, inherited, viaView, aliasInfo) // scala 2.13.x
                          if !sym.isConstructor /*&& nameMatches(sym)*/ =>
                        elb += pcompiler.mkCompletionProposal(sym, tpe, inherited, viaView)
                      case pcompiler.ScopeMember(sym, tpe, true, _, aliasInfo) if !sym.isConstructor /*&& nameMatches(sym)*/ => // scala 2.13.x
